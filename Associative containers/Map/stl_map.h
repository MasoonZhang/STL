...
namespace STL {
	...
	public:
		...
		map(const map<Key, T, Compare, Alloc>& operator=(const map(const map<Key, T, Compare, Alloc>& x) {
			t = x.t;
			return *this;
		}
	...
	inline bool operator==(const map<Key, Tp, Compare, Alloc>& x, const map<Key, Tp, Compare, Alloc>& y) {
		return x.t == y.t;
	}
