...
namespace STL {
	...
	OutputIterator adjacent_difference(InputIterator first, InputIterator last, OutputIterator result) {
		...
		*result = *first;
		__adjacent_difference(first, last, result, value_type(first));
	}
	...
	OutputIterator adjacent_difference(InputIterator first, InputIterator last, OutputIterator result, BinaryOperation binary_op) {
		if (first == last) return result;
		*result = *first;
		__adjacent_difference(first, last, result, value_type(first), binary_op);
	}
	...
	T accumulate(InputIterator1 first1, InputIterator1 last1, InputIterator2 first2, T init) {
	...
	T accumulate(InputIterator1 first1, InputIterator1 last1, InputIterator2 first2, T init, BinaryOperation1 binary_op1, BinaryOperation2 binary_op2) {
		for (; first1 != last1; ++first1, ++first2)
	...
	OutputIterator partial_sum(InputIterator first, InputIterator last, OutputIterator result) {
		if (first == last) return result;
		*result = *first;
		__partial_sum(first, last, result, value_type(first));
	}
